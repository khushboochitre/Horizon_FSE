...
static bool NoCheckForDynAAMetadata = false;

STATISTIC(NumMust,  "Number of must-alias pointer pairs");
STATISTIC(NumNo,  "Number of no-alias pointer pairs");
STATISTIC(NumMay,  "Number of may-alias pointer pairs");
STATISTIC(NumPartial,  "Number of partial-alias pointer pairs");
STATISTIC(MissedNumNo,  "Number of no-alias pointer pairs missed by alias analysis");
STATISTIC(NumPointerPairs,  "Number of pointer pairs");
STATISTIC(MissedQueries,  "Number of missed queries due to size condition");

/// Allow disabling BasicAA from the AA results. This is particularly useful
/// when testing to isolate a single AA implementation.
static cl::opt<bool> DisableBasicAA("disable-basicaa", cl::Hidden,
                                    cl::init(false));
static cl::opt<bool> DisableGetDynAA("disable-get-dynaa",
                       cl::init(false));
...
AliasResult AAResults::alias(const MemoryLocation &LocA,
  ...
  AAQueryInfo AAQIP;
  NumPointerPairs++;
  AliasResult Result = alias(LocA, LocB, AAQIP);
  if(Result == MayAlias && !DisableGetDynAA) {
    if (Instruction *inst = dyn_cast<Instruction>(const_cast<Value *>(LocA.Ptr))) {
      if (inst->getFunction()) {
        Function *F = inst->getFunction();
        if (F && F->hasFnAttribute("noalias_dynaa_intrinsic") && LocA.Ptr != LocB.Ptr && !NoCheckForDynAAMetadata) {
          NoCheckForDynAAMetadata = true;
          SLVAPass SLVA;
          //Result = SLVA.checkAliasMetadata(F, const_cast<Value *>(LocA.Ptr), const_cast<Value *>(LocB.Ptr));
          unsigned MissedQ = 0;
          Result = SLVA.checkAliasMetadata(F, LocA, LocB, this, &MissedQ);
          MissedQueries = MissedQueries + MissedQ;
          NoCheckForDynAAMetadata = false;
        }
      }
    }

    if (Argument *arg = dyn_cast<Argument>(const_cast<Value *>(LocA.Ptr))) {
      if (arg->getParent()) {
