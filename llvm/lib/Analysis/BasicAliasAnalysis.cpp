...
//#include "llvm/Analysis/GetDynamicAAResults.h"
#include "llvm/Support/Debug.h"
#include "llvm/IR/LegacyPassManager.h"
//#include "llvm/Analysis/UseDynamicAAResults.h"
...
//static cl::opt<bool> DynamicAAResults("dynaa-results",
//                                      cl::init(false));
...
AliasResult BasicAAResult::alias(const MemoryLocation &LocA,
  ...
  //errs() << "BasicAA " << Alias << " " << *LocA.Ptr << "<->" << *LocB.Ptr << "\n";
  VisitedPhiBBs.clear();
  return Alias;
}
...
AliasResult BasicAAResult::aliasCheck(const Value *V1, LocationSize V1Size,
  ...
  /*if(Result == MayAlias) {
    if(const Function *F = getParent(V1)) {
  ...
  /*if(Result == MayAlias) {
    const Function *F = getParent(V1);
    if (F && F->hasFnAttribute("noalias_dynaa_intrinsic") && V1 != V2) {
      SLVAPass SLVA;
      //Result = SLVA.createNoAliasMap(const_cast<Function *>(F), const_cast<Value *>(V1), const_cast<Value *>(V2));
      Result = SLVA.checkAliasMetadata(const_cast<Function *>(F), const_cast<Value *>(V1), const_cast<Value *>(V2));
    }
  }*/

  /*if (const Function *F = getParent(V1)) {
  	if (Result == MayAlias && F->hasFnAttribute("dynaa_fn_noalias") && V1 != V2 && !inNoAliasContext) {
			inNoAliasContext = 1;
      SLVAPass SLVA;
      Result = SLVA.isNoAliasDynAA(this, AAQI, const_cast<Function *>(F), const_cast<Value *>(V1), const_cast<Value *>(V2));
			inNoAliasContext = 0;
  	}
  }

	if (inNoAliasContext && Result == MayAlias) {
		return Result;
	}*/
  
  /*if(Result == MayAlias && DynamicAAResults) {
    GetDynamicAAResults GDA;
    //if(UDA.useDyn) {
    if (GDA.isEnabledDynAA()) {
      if(const Function *F = getParent(V1)) {
        //Function *Func = const_cast<Function *>(F);
        //legacy::PassManager PM;
        //PM.add(new UseDynamicAAResults());
        //PM.run(*Func->getParent());
        UseDynamicAAResults UDA;
        Result = UDA.alias(this, F, V1, V2);
        //errs() << F->getName() << " " << *V1 << " " << *V2 << "\n";
      }
    }
  }*/
