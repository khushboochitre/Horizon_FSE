...
static cl::opt<bool> DisableGVN("disable-gvn", cl::init(false));
static cl::opt<bool> VectEnabled("loop-vect-enabled", cl::init(false));
...
SpeculationFailure:
  ...
  do {
    ...
    if (EntryVal == 0 || EntryVal == 1) continue;  // Already unavailable.
...
void GVN::fixIRForVectorization(Function *F, PHINode *PHI) {
  ...
  if(firstUserFound) {
    ...
    Instruction *InsertHere = firstUser;
...
bool GVN::processNonLocalLoad(LoadInst *LI) {
  ...
  unsigned num_of_start_marker = 0;
  //Computes number of dependencies on start marker.
  for (unsigned i = 0, e = NumDeps; i != e; ++i) {
    MemDepResult DepInfo = Deps[i].getResult();
    if(DepInfo.getInst()) {
      if (IntrinsicInst* II = dyn_cast<IntrinsicInst>(DepInfo.getInst())) {
  ...
  if(ValuesPerBlock.size() == num_of_start_marker) {
    ValuesPerBlock.clear();
  }
