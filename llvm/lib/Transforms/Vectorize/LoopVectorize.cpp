...
void InnerLoopVectorizer::fixFirstOrderRecurrence(PHINode *Phi) {
  ...
  std::vector<Value *> ShuffleVectors;
  ...
  for (unsigned Part = 0; Part < UF; ++Part) {
    ...
    ShuffleVectors.push_back(Shuffle);
  }

  /*for(auto SV : ShuffleVectors) {
    if(ShuffleVectorInst *SVI = dyn_cast<ShuffleVectorInst>(SV)) {
      std::vector<Instruction *> moveInstructions;
      for (User *U : SVI->users()) {
        if (ShuffleVectorInst *SVII = dyn_cast<ShuffleVectorInst>(U)) {
          moveInstructions.push_back(SVII);
        }
      }

      for(auto MI : moveInstructions) { 
        BasicBlock *CurBB = SVI->getParent(); 
